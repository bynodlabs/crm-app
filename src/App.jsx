...
function WhatsAppApiView({ isDarkMode = false, sessionToken = null, currentUser = null, onVerifySession = null }) {
  ...
  const handleRestartQrSession = useCallback(async () => {
    setQrFeedback('');
    await handleDisconnectQr();
  }, [handleDisconnectQr]);
  ...
  const renderQrContent = () => {
            ...
            <div className={`flex flex-col items-center gap-3 rounded-[1.4rem] border px-5 py-4 shadow-[0_18px_48px_-26px_rgba(15,23,42,0.35)] ${isDarkMode ? 'border-white/10 bg-[#121212]/95 text-white' : 'border-slate-200 bg-white text-slate-900'}`}>
              ...
              <div className="flex flex-col items-center gap-2">
                <button
                  ...
                  disabled={isQrBusy}
                  className={`relative overflow-hidden rounded-full bg-[linear-gradient(135deg,#FF3C00,#FF7A00_60%,#FFB36B)] px-4 py-2 text-xs font-black text-white shadow-[0_14px_28px_-18px_rgba(255,90,31,0.55)] transition-transform hover:-translate-y-0.5 ${isQrBusy ? 'cursor-not-allowed opacity-70' : 'animate-pulse'}`}
                >
                  <span className="pointer-events-none absolute inset-0 bg-[linear-gradient(120deg,transparent,rgba(255,255,255,0.24),transparent)]" />
                  {isQrBusy ? 'Cargando...' : 'Generar nuevo código'}
                </button>
                <button
                  type="button"
                  onClick={handleRestartQrSession}
                  disabled={isQrBusy}
                  className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-[11px] font-bold transition-colors ${isDarkMode ? 'text-slate-300 hover:bg-white/5 hover:text-white' : 'text-slate-500 hover:bg-orange-50 hover:text-[#FF5A1F]'} ${isQrBusy ? 'cursor-not-allowed opacity-60' : 'animate-pulse'}`}
                >
                  <span className="h-2 w-2 rounded-full bg-[#FF5A1F]" />
                  <span className="whitespace-nowrap">Desconectar y volver a vincular</span>
                </button>
              </div>
            </div>
          </div>
        ...
        <div className={`wa-qr-shell relative mx-auto flex w-full max-w-[520px] flex-col items-center overflow-hidden rounded-[1.65rem] border px-3.5 py-3.5 transition-transform ${isDarkMode ? 'border-white/10 bg-[#0d1012]' : 'border-slate-200 bg-[radial-gradient(circle_at_top,rgba(241,245,249,0.95),rgba(255,255,255,1))]'}`}>
          <div className={`relative mx-auto w-fit rounded-[1.45rem] border p-3.5 ${isDarkMode ? 'border-white/10 bg-[#111317]' : 'border-slate-200 bg-white'}`}>
            <div className={`wa-qr-placeholder flex h-[200px] w-[200px] items-center justify-center rounded-[1.2rem] border border-dashed sm:h-[228px] sm:w-[228px] ${
              ...
              <div className="text-center">
                <p className="text-sm font-bold">QR no disponible</p>
                <p className="mt-1 text-xs">Inicia o verifica tu sesion para generarlo.</p>
              </div>
            </div>
          </div>
      ...
      return (
        <div className={`wa-qr-min flex min-h-[232px] items-center justify-center rounded-[1.65rem] border p-4 ${isDarkMode ? 'border-emerald-400/20 bg-emerald-500/10' : 'border-emerald-200 bg-emerald-50/80'}`}>
          <div className={`w-full max-w-[300px] rounded-[1.45rem] border p-5 text-center shadow-[0_18px_48px_-30px_rgba(16,185,129,0.45)] ${isDarkMode ? 'border-white/10 bg-[#111317] text-white' : 'border-white bg-white text-slate-900'}`}>
            ...
            >
              {isQrBusy ? 'Desconectando...' : 'Desconectar'}
            </button>
          </div>
        </div>
      );
    ...
    return (
      <div className={`wa-qr-shell relative mx-auto w-full max-w-[520px] overflow-hidden rounded-[1.65rem] border px-3.5 py-3.5 transition-transform ${isDarkMode ? 'border-white/10 bg-[#0d1012]' : 'border-slate-200 bg-[radial-gradient(circle_at_top,rgba(241,245,249,0.95),rgba(255,255,255,1))]'}`}>
        ...
        {qrFeedback ? (
          <p className={`mt-3 text-center text-[11px] ${mutedText}`}>{qrFeedback}</p>
        ) : null}
      </div>
    );
  };
...
function CleanDashboardView({
  ...
  const usuariosOperativos = useMemo(
    () => mergeUniqueUsers(usersDb).filter((user) => user.email !== 'bynodlabs@gmail.com'),
    [usersDb],
  );
